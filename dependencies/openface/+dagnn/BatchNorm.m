    ...
    function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)
        [derInputs{1}, derParams{1}, derParams{2}, derParams{3}] = ...
            vl_nnbnorm(inputs{1}, params{1}, params{2}, derOutputs{1}, ...
                   ...
                   obj.opts{:}) ;
        if ~isempty(params{3})
            % there's an issue with the backward pass when moments are
            % provided. this code computes it correctly.
            derX = single(ones(size(derInputs{1})));
            derX = bsxfun(@times, derX, permute(params{1}, [3 1 2]));
            mmts = params{3};
            sigma = permute(mmts(:,2), [3 2 1]);
            derX = bsxfun(@rdivide, derX, (obj.epsilon+sigma));
            derInputs{1} = derX.*derOutputs{1};
        end
      obj.moments = [] ;
