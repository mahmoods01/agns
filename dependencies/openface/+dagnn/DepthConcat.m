classdef DepthConcat < dagnn.ElementWise
  properties
    dim = 3
  end
  ...
  methods
    function outputs = forward(obj, inputs, params)
      outputs{1} = vl_nndepthconcat(inputs, obj.dim) ;
      obj.inputSizes = cellfun(@size, inputs, 'UniformOutput', false) ;
    end

    function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)
      derInputs = vl_nndepthconcat(inputs, obj.dim, derOutputs{1}, 'inputSizes', obj.inputSizes) ;
      derParams = {} ;
    end
    ...
    function obj = DepthConcat(varargin)
      obj.load(varargin{:}) ;
    end
  end
end
