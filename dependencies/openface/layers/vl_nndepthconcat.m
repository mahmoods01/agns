function y = vl_nndepthconcat(inputs, dim, dzdy, varargin)
%VL_NNCONCAT CNN concatenate multiple inputs.
%  Y = VL_NNCONCAT(INPUTS, DIM) concatenates the inputs in the cell
%  array INPUTS along dimension DIM generating an output Y.
%
%  DZDINPUTS = VL_NNCONCAT(INPUTS, DIM, DZDY) computes the derivatives
%  of the block projected onto DZDY. DZDINPUTS has one element for
%  each element of INPUTS, each of which is an array that has the same
%  dimensions of the corresponding array in INPUTS.

% Copyright (C) 2015 Karel Lenc and Andrea Vedaldi.
% All rights reserved.
%
% This file is part of the VLFeat library and is made available under
% the terms of the BSD license (see the COPYING file).

opts.inputSizes = [] ;
opts = vl_argparse(opts, varargin, 'nonrecursive');

if nargin < 2, dim = 3; end;
if nargin < 3, dzdy = []; end;

if isempty(dzdy)
  try
    y = cat(dim, inputs{:});
  catch ME
    %Pad assuming all are squares
    all_sizes = [];
    for i = 1:size(inputs, 2)
      this_size = size(inputs{i}, 1);
      all_sizes = [all_sizes this_size];
    end
    [max_size, max_idx] = max(all_sizes);
     
    for i = 1:size(inputs, 2)
      this_size = size(inputs{i}, 1);
      %Assume this is always an even number (diff)
      diff = max_size - this_size;
      pad = diff/2;
      
      temp_input = zeros(max_size, max_size, size(inputs{i}, 3), size(inputs{i}, 4));
      if isa(inputs{i}, 'single')
          temp_input = single(temp_input);
      end
      if isa(inputs{i}, 'gpuArray')
          temp_input = gpuArray(single(temp_input));
      end
      temp_input((1+floor(pad)):(max_size-ceil(pad)), (1+floor(pad)):(max_size-ceil(pad)), :, :) = inputs{i};
      inputs{i} = temp_input;
    end
    y = cat(dim, inputs{:});
  end
else
  if isempty(opts.inputSizes)
    opts.inputSizes = cellfun(@(inp) [size(inp,1),size(inp,2),size(inp,3),size(inp,4)], inputs, 'UniformOutput', false) ;
  end
  start = 1 ;
  y = cell(1, numel(opts.inputSizes)) ;
  s.type = '()' ;
  s.subs = {':', ':', ':', ':'} ;
  
  all_sizes = [];
  for i = 1:size(inputs, 2)
    this_size = size(inputs{i}, 1);
    all_sizes = [all_sizes this_size];
  end
  [max_size, max_idx] = max(all_sizes);
  
  for i = 1:numel(opts.inputSizes)
    stop = start + opts.inputSizes{i}(dim) ;
    s.subs{dim} = start:stop-1 ;
    y{i} = subsref(dzdy,s) ;
    start = stop ;
    
    this_size = opts.inputSizes{i}(1);
    diff = max_size - this_size;
    pad = diff/2;
    
    y{i} = y{i}((1+floor(pad)):(max_size-ceil(pad)), (1+floor(pad)):(max_size-ceil(pad)), :, :);
  end
end
